...
BOOSTVERSION=$( grep "#define BOOST_VERSION" "/usr/local/include/boost/version.hpp" 2>/dev/null | tail -1 | tr -s ' ' | cut -d\  -f3 )
if [ "${BOOSTVERSION}" != "${BOOST_VERSION_MAJOR}0${BOOST_VERSION_MINOR}0${BOOST_VERSION_PATCH}" ]; then
	printf "Installing Boost library...\\n"
	BREW_BOOST_VERSION=${BOOST_VERSION_MAJOR}.${BOOST_VERSION_MINOR}.${BOOST_VERSION_PATCH}_1
	curl -LC - https://bintray.com/homebrew/bottles/download_file?file_path=boost-${BREW_BOOST_VERSION}.mojave.bottle.tar.gz -o boost-${BREW_BOOST_VERSION}.mojave.bottle.tar.gz \
	&& tar -xzf boost-${BREW_BOOST_VERSION}.mojave.bottle.tar.gz \
	&& mkdir -p /usr/local/Cellar/boost/ \
	&& mv boost/${BREW_BOOST_VERSION} /usr/local/Cellar/boost/ \
	&& brew switch boost ${BREW_BOOST_VERSION} \
	&& rmdir boost && rm -f boost_${BREW_BOOST_VERSION}.tar.gz \
	|| exit 1
	printf " - Boost library successfully installed @ ${BREW_BOOST_VERSION}.\\n"
else
	printf " - Boost library found with correct version @ ${BOOST_VERSION}.\\n"
fi
