...
#include <pose/chain/permission_link_object.hpp>
 ...
 */
void apply_pose_newaccount(apply_context& context) {
...
//   context.require_write_lock( config::eosio_auth_scope );
   ...
   if( !creator.privileged ) {
      EOS_ASSERT( name_str.find( "pose." ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with 'pose.'" );
   }
...
void apply_pose_setcode(apply_context& context) {
...
void apply_pose_setabi(apply_context& context) {
   auto& db  = context.db;
...
void apply_pose_updateauth(apply_context& context) {
   ...
   EOS_ASSERT( update.permission.to_string().find( "pose." ) != 0, action_validate_exception,
               "Permission names that start with 'pose.' are reserved" );
...
void apply_pose_deleteauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
...
void apply_pose_linkauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
...
void apply_pose_unlinkauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
...
void apply_pose_canceldelay(apply_context& context) {
