...
const static uint64_t system_account_name    = N(pose);
const static uint64_t null_account_name      = N(pose.null);
const static uint64_t producers_account_name = N(pose.prods);

// Active permission of producers account requires greater than 2/3 of the producers to authorize
const static uint64_t majority_producers_permission_name = N(pose.major); // greater than 1/2 of producers needed to authorize
const static uint64_t minority_producers_permission_name = N(pose.minor); // greater than 1/3 of producers needed to authorize0

const static uint64_t eosio_auth_scope       = N(pose.auth);
const static uint64_t eosio_all_scope        = N(pose.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t eosio_any_name = N(pose.any);
const static uint64_t eosio_code_name = N(pose.code);
