...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   try {
      try {
         genesis_state gs; // Check if POSE_ROOT_KEY is bad
      } catch ( const fc::exception& ) {
         elog( "POSE_ROOT_KEY ('${root_key}') is invalid. Recompile with a valid public key.",
               ("root_key", genesis_state::pose_root_key));
         throw;
      }
